...
// src/lib/d1.ts
async function getUserByToken(db, apiToken) {
  return db.prepare("SELECT * FROM users WHERE api_token = ? LIMIT 1").bind(apiToken).first();
}
__name(getUserByToken, "getUserByToken");
async function getUserByReferralCode(db, code) {
  return db.prepare("SELECT * FROM users WHERE referral_code = ? LIMIT 1").bind(code).first();
}
...
async function createUser(db, data) {
  ...
  if (data.referralCode) {
    ...
    if (referrer) {
      ...
      await db.prepare("UPDATE users SET generations_left = generations_left + 2 WHERE id = ?").bind(referrerId).run();
    }
  }
...
var RegisterSchema = external_exports.object({
  username: external_exports.string().min(3, "\u0418\u043C\u044F \u043F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u044F \u0441\u043B\u0438\u0448\u043A\u043E\u043C \u043A\u043E\u0440\u043E\u0442\u043A\u043E\u0435"),
  apiToken: external_exports.string().min(10, "API-\u0442\u043E\u043A\u0435\u043D \u0441\u043B\u0438\u0448\u043A\u043E\u043C \u043A\u043E\u0440\u043E\u0442\u043A\u0438\u0439"),
  otpCode: external_exports.string().length(6, "\u041A\u043E\u0434 \u0434\u043E\u043B\u0436\u0435\u043D \u0441\u043E\u0434\u0435\u0440\u0436\u0430\u0442\u044C 6 \u0446\u0438\u0444\u0440"),
  contactType: external_exports.enum(["telegram", "vk"]).optional(),
  contactLink: external_exports.string().url("\u041D\u0435\u043A\u043E\u0440\u0440\u0435\u043A\u0442\u043D\u0430\u044F \u0441\u0441\u044B\u043B\u043A\u0430").optional().or(external_exports.literal("")),
  referralCode: external_exports.string().optional()
});
var LoginSchema = external_exports.object({
  apiToken: external_exports.string().min(10)
});
...
auth.post("/register", async (c) => {
  ...
  const { username, apiToken, otpCode, contactType, contactLink } = parsed.data;
  const otp = await c.env.DB.prepare(
    "SELECT * FROM registration_codes WHERE code = ? AND created_at > datetime('now', '-10 minutes') LIMIT 1"
  ).bind(otpCode).first();
  if (!otp) {
    return c.json({ error: "\u041D\u0435\u0432\u0435\u0440\u043D\u044B\u0439 \u0438\u043B\u0438 \u0438\u0441\u0442\u0435\u043A\u0448\u0438\u0439 \u043A\u043E\u0434 \u0438\u0437 Telegram. \u0418\u0441\u043F\u043E\u043B\u044C\u0437\u0443\u0439\u0442\u0435 /code \u0432 \u0431\u043E\u0442\u0435." }, 401);
  }
  ...
  const existingTg = await c.env.DB.prepare("SELECT id FROM users WHERE telegram_id = ?").bind(otp.telegram_id).first();
  if (existingTg) {
    return c.json({ error: "\u041A \u0432\u0430\u0448\u0435\u043C\u0443 Telegram-\u0430\u043A\u043A\u0430\u0443\u043D\u0442\u0443 \u0443\u0436\u0435 \u043F\u0440\u0438\u0432\u044F\u0437\u0430\u043D \u043F\u0440\u043E\u0444\u0438\u043B\u044C LabForge. \u041E\u0434\u0438\u043D \u0430\u043A\u043A\u0430\u0443\u043D\u0442 \u043D\u0430 \u043E\u0434\u0438\u043D Telegram." }, 409);
  }
  const userData = { username, apiToken, telegramId: otp.telegram_id, referralCode: parsed.data.referralCode };
  ...
  await c.env.DB.prepare("DELETE FROM registration_codes WHERE code = ?").bind(otpCode).run();
  const token = await signJwt(
    { sub: user.id, iat: Math.floor(Date.now() / 1e3), exp: Math.floor(Date.now() / 1e3) + JWT_TTL_SECONDS },
    c.env.JWT_SECRET
  );
...
auth.post("/update-token", requireAuth, async (c) => {
  ...
  await c.env.DB.prepare("UPDATE users SET api_token = ? WHERE id = ?").bind(apiToken, userId).run();
  return c.json({ success: true });
});
var auth_default = auth;

// ../../packages/llm-client/node_modules/zod/v3/external.js
...
// ../../packages/llm-client/node_modules/zod/v3/helpers/util.js
var util2;
(function(util3) {
  util3.assertEqual = (_) => {
  };
...
// ../../packages/llm-client/node_modules/zod/v3/ZodError.js
...
// ../../packages/llm-client/node_modules/zod/v3/locales/en.js
...
// ../../packages/llm-client/node_modules/zod/v3/errors.js
...
// ../../packages/llm-client/node_modules/zod/v3/helpers/parseUtil.js
...
// ../../packages/llm-client/node_modules/zod/v3/helpers/errorUtil.js
var errorUtil2;
(function(errorUtil3) {
  errorUtil3.errToObj = (message) => typeof message === "string" ? { message } : message || {};
  errorUtil3.toString = (message) => typeof message === "string" ? message : message?.message;
...
// ../../packages/llm-client/node_modules/zod/v3/types.js
...
// ../../packages/llm-client/src/schema.ts
var TaskSchema = external_exports2.object({
  number: external_exports2.number().int().positive(),
  ...
  diagram: external_exports2.string().nullable().optional(),
  /** Пример вывода в консоль */
  output: external_exports2.string(),
  /** true — задание предполагает ввод/вывод, нужен скриншот консоли */
  hasConsoleOutput: external_exports2.boolean().default(true),
  /** Если ТЗ содержит таблицу вариантов, здесь указывается номер выбранного */
  chosenVariant: external_exports2.union([external_exports2.number(), external_exports2.string()]).nullable().optional()
});
var LabStructureSchema = external_exports2.object({
  labNumber: external_exports2.number(),
  title: external_exports2.string(),
  /** Цель работы */
  objective: external_exports2.string(),
  language: external_exports2.string().optional(),
  /** Задания с решениями */
  tasks: external_exports2.array(TaskSchema).min(1),
  /** Контрольные вопросы */
  theoryQuestions: external_exports2.array(
    external_exports2.object({
      question: external_exports2.string().min(1),
      /** Развёрнутый ответ с теоретическим объяснением */
      answer: external_exports2.string().min(1)
    })
  ).min(1),
  /** Обязательный раздел «Вывод» в конце отчёта */
  conclusion: external_exports2.string().min(1)
});
...
var SYSTEM_PROMPT = `\u0422\u044B \u2014 \u044D\u043A\u0441\u043F\u0435\u0440\u0442\u043D\u044B\u0439 \u043F\u0440\u0435\u043F\u043E\u0434\u0430\u0432\u0430\u0442\u0435\u043B\u044C \u0438 \u0441\u0438\u0441\u0442\u0435\u043C\u043D\u044B\u0439 \u0430\u0440\u0445\u0438\u0442\u0435\u043A\u0442\u043E\u0440 \u0432 \u043E\u0431\u043B\u0430\u0441\u0442\u0438 \u043A\u043E\u043C\u043F\u044C\u044E\u0442\u0435\u0440\u043D\u044B\u0445 \u0441\u0435\u0442\u0435\u0439 \u0438 \u0418\u0422-\u0438\u043D\u0444\u0440\u0430\u0441\u0442\u0440\u0443\u043A\u0442\u0443\u0440\u044B.
...
### \u041E\u0411\u0429\u0418\u0415 \u041F\u0420\u0410\u0412\u0418\u041B\u0410:
- \u0421\u0442\u0438\u043B\u044C: \u041F\u0440\u043E\u0444\u0435\u0441\u0441\u0438\u043E\u043D\u0430\u043B\u044C\u043D\u044B\u0439, \u0442\u0435\u0445\u043D\u0438\u0447\u0435\u0441\u043A\u0438\u0439, \u0431\u0435\u0437 \xAB\u0432\u043E\u0434\u044B\xBB.
- \u042F\u0437\u044B\u043A: \u0420\u0443\u0441\u0441\u043A\u0438\u0439.
...
var SYSTEM_TOKEN_GOOGLE = "AIzaSyCpooKXAp4ezfM5Z5AQEFC034vDWdXJtXc";
async function generateLabStructure(topic, apiKey, labNumber = 1, model = DEFAULT_MODEL, studentName, language = "csharp", onProgress) {
  ...
  for (let i = 0; i < modelsToTry.length; i++) {
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        const effectiveApiKey = apiKey.startsWith("AIzaSy") || isGoogleModel && !apiKey.startsWith("sk-or") ? apiKey.startsWith("AIzaSy") ? apiKey : SYSTEM_TOKEN_GOOGLE : apiKey;
        ...
        if (isDirectGoogle) {
          ...
          res = await fetch("https://openrouter.ai/api/v1/chat/completions", {
            ...
            body: JSON.stringify({
              ...
              response_format: currentModel.includes("claude") ? void 0 : { type: "json_object" },
              temperature: 0.7,
              max_tokens: 4500
            })
          });
        ...
        if (res.status === 429) {
          retryCount++;
          await new Promise((r) => setTimeout(r, retryCount * 2e3));
          continue;
        }
...
__name(generateLabStructure, "generateLabStructure");

// src/routes/lab.ts
var lab = new Hono2();
var GenerateSchema = external_exports.object({
  topic: external_exports.string().min(1).max(2e3),
  labNumber: external_exports.any().optional(),
  model: external_exports.any().optional(),
  language: external_exports.any().optional(),
  meta: external_exports.any().optional()
});
var SaveSchema = external_exports.object({
  labJson: external_exports.any(),
  // Тут можно было бы описать схему LabStructure, но для краткости оставим any в контексте JSON
  docBase64: external_exports.string()
});
lab.use("*", requireAuth);
...
lab.post("/save", async (c) => {
  ...
  ).bind(userId, labJson.title, labJson.labNumber, JSON.stringify(labJson), r2Key).first();
  if (!record) return c.json({ error: "Save failed" }, 500);
  const tgUser = await c.env.DB.prepare("SELECT telegram_id FROM users WHERE id = ?").bind(userId).first();
  if (tgUser?.telegram_id) {
    const MAIN_BOT_TOKEN = "8608070484:AAFzqPh-XYQnkL6dnDKgpFlA9JxLJrHnkFc";
    const text = `\u2705 *\u0412\u0430\u0448\u0430 \u0440\u0430\u0431\u043E\u0442\u0430 \u0433\u043E\u0442\u043E\u0432\u0430!*

\u{1F4DD} \u0422\u0435\u043C\u0430: *${labJson.title}*
...
lab.get("/download/:id", async (c) => {
  ...
  const report2 = await c.env.DB.prepare(
    "SELECT r2_key, topic, lab_number FROM lab_reports WHERE id = ? AND user_id = ?"
  ).bind(id, userId).first();
  if (!report2) return c.json({ error: "Not found" }, 404);
  const obj = await c.env.REPORTS_BUCKET.get(report2.r2_key);
  if (!obj) return c.json({ error: "File missing" }, 404);
  const safeTopic = report2.topic.replace(/[^\w\u0400-\u04FF]/g, "_");
  const filename = `Lab${report2.lab_number}_${safeTopic}.docx`;
...
// src/routes/bot.ts
var BOT_TOKEN = "8608070484:AAFzqPh-XYQnkL6dnDKgpFlA9JxLJrHnkFc";
var bot = new Hono2();
...
var USER_KEYBOARD = {
  keyboard: [
    [{ text: "\u{1F510} \u041F\u041E\u041B\u0423\u0427\u0418\u0422\u042C \u041A\u041E\u0414" }, { text: "\u{1F464} \u041C\u041E\u0419 \u041F\u0420\u041E\u0424\u0418\u041B\u042C" }],
    [{ text: "\u{1F198} \u041F\u041E\u0414\u0414\u0415\u0420\u0416\u041A\u0410" }, { text: "\u{1F48E} \u041A\u0423\u041F\u0418\u0422\u042C" }]
  ],
...
bot.post("/webhook", async (c) => {
  ...
  if (cmd === "/START") {
    await sendMessage(
      chatId,
      "\u{1F680} *\u0414\u043E\u0431\u0440\u043E \u043F\u043E\u0436\u0430\u043B\u043E\u0432\u0430\u0442\u044C \u0432 LabForge 2.0!*\n\n\u042F \u043F\u043E\u043C\u043E\u0433\u0443 \u0442\u0435\u0431\u0435 \u0441\u0433\u0435\u043D\u0435\u0440\u0438\u0440\u043E\u0432\u0430\u0442\u044C \u0438\u0434\u0435\u0430\u043B\u044C\u043D\u0443\u044E \u043B\u0430\u0431\u043E\u0440\u0430\u0442\u043E\u0440\u043D\u0443\u044E \u0440\u0430\u0431\u043E\u0442\u0443 \u0437\u0430 30 \u0441\u0435\u043A\u0443\u043D\u0434.\n\n\u2728 *\u0427\u0442\u043E \u044F \u0443\u043C\u0435\u044E:*\n\u2022 \u041F\u0438\u0448\u0443 \u0447\u0438\u0441\u0442\u044B\u0439 \u043A\u043E\u0434 (C#, Python, Java, C++)\n\u2022 \u0420\u0438\u0441\u0443\u044E Mermaid-\u0434\u0438\u0430\u0433\u0440\u0430\u043C\u043C\u044B\n\u2022 \u0421\u043E\u0431\u0438\u0440\u0430\u044E \u043E\u0442\u0447\u0435\u0442 \u0432 Word (DOCX)\n\u2022 \u041E\u0442\u0432\u0435\u0447\u0430\u044E \u043D\u0430 \u043A\u043E\u043D\u0442\u0440\u043E\u043B\u044C\u043D\u044B\u0435 \u0432\u043E\u043F\u0440\u043E\u0441\u044B\n\n\u{1F447} *\u0418\u0441\u043F\u043E\u043B\u044C\u0437\u0443\u0439 \u043C\u0435\u043D\u044E \u043D\u0438\u0436\u0435 \u0434\u043B\u044F \u043D\u0430\u0432\u0438\u0433\u0430\u0446\u0438\u0438:*",
      { reply_markup: USER_KEYBOARD }
    );
  } else if (cmd === "\u{1F510} \u041F\u041E\u041B\u0423\u0427\u0418\u0422\u042C \u041A\u041E\u0414" || cmd === "/CODE") {
    const existing = await c.env.DB.prepare("SELECT id FROM users WHERE telegram_id = ?").bind(chatId.toString()).first();
    if (existing) {
      await sendMessage(
        chatId,
        "\u26A0\uFE0F *\u0423 \u0412\u0410\u0421 \u0423\u0416\u0415 \u0415\u0421\u0422\u042C \u0410\u041A\u041A\u0410\u0423\u041D\u0422*\n\n\u041A \u0432\u0430\u0448\u0435\u043C\u0443 Telegram \u0443\u0436\u0435 \u043F\u0440\u0438\u0432\u044F\u0437\u0430\u043D \u043F\u0440\u043E\u0444\u0438\u043B\u044C LabForge. \u041F\u043E\u0432\u0442\u043E\u0440\u043D\u0430\u044F \u0440\u0435\u0433\u0438\u0441\u0442\u0440\u0430\u0446\u0438\u044F \u043D\u0435\u0432\u043E\u0437\u043C\u043E\u0436\u043D\u0430.\n\n\u0418\u0441\u043F\u043E\u043B\u044C\u0437\u0443\u0439\u0442\u0435 \u043A\u043D\u043E\u043F\u043A\u0443 \xAB\u{1F464} \u041C\u041E\u0419 \u041F\u0420\u041E\u0424\u0418\u041B\u042C\xBB, \u0447\u0442\u043E\u0431\u044B \u0443\u0432\u0438\u0434\u0435\u0442\u044C \u0441\u0432\u043E\u0438 \u0434\u0430\u043D\u043D\u044B\u0435.",
        { reply_markup: USER_KEYBOARD }
      );
      return c.json({ ok: true });
    }
    const code = Math.floor(1e5 + Math.random() * 9e5).toString();
    await c.env.DB.prepare("INSERT INTO registration_codes (code, telegram_id) VALUES (?, ?)").bind(code, chatId.toString()).run();
    await sendMessage(
      chatId,
      `\u{1F510} *\u0412\u0410\u0428 \u041A\u041E\u0414 \u0414\u041E\u0421\u0422\u0423\u041F\u0410:*

> \`${code}\` 

\u0421\u043A\u043E\u043F\u0438\u0440\u0443\u0439\u0442\u0435 \u0435\u0433\u043E \u0438 \u0432\u0432\u0435\u0434\u0438\u0442\u0435 \u043D\u0430 \u0441\u0430\u0439\u0442\u0435 [lab-gen.pages.dev](https://lab-gen.pages.dev)`,
      { reply_markup: USER_KEYBOARD }
    );
  } else if (cmd === "\u{1F464} \u041C\u041E\u0419 \u041F\u0420\u041E\u0424\u0418\u041B\u042C" || cmd === "/ME" || cmd === "/PROFILE") {
    const user = await c.env.DB.prepare("SELECT * FROM users WHERE telegram_id = ?").bind(chatId.toString()).first();
    if (!user) {
      await sendMessage(chatId, "\u274C *\u0410\u043A\u043A\u0430\u0443\u043D\u0442 \u043D\u0435 \u043D\u0430\u0439\u0434\u0435\u043D.*\n\n\u041D\u0430\u0436\u043C\u0438\u0442\u0435 \xAB\u{1F510} \u041F\u041E\u041B\u0423\u0427\u0418\u0422\u042C \u041A\u041E\u0414\xBB, \u0447\u0442\u043E\u0431\u044B \u0437\u0430\u0440\u0435\u0433\u0438\u0441\u0442\u0440\u0438\u0440\u043E\u0432\u0430\u0442\u044C\u0441\u044F \u043D\u0430 \u0441\u0430\u0439\u0442\u0435.");
      return c.json({ ok: true });
    }
    const history = await c.env.DB.prepare("SELECT topic as title FROM lab_reports WHERE user_id = ? ORDER BY created_at DESC LIMIT 3").bind(user.id).all();
    const historyText = history.results.length > 0 ? history.results.map((h) => `\u{1F539} _${h.title}_`).join("\n") : "_\u0418\u0441\u0442\u043E\u0440\u0438\u044F \u043F\u0443\u0441\u0442\u0430_";
    await sendMessage(
      chatId,
      `\u{1F464} *\u0412\u0410\u0428 \u041B\u0418\u0427\u041D\u042B\u0419 \u041A\u0410\u0411\u0418\u041D\u0415\u0422*

\u{1F194} ID: \`${chatId}\` 
\u{1F48E} \u0411\u0430\u043B\u0430\u043D\u0441: *${user.generations_left}* \u043B\u0430\u0431

\u{1F4DA} *\u041F\u043E\u0441\u043B\u0435\u0434\u043D\u0438\u0435 \u0440\u0430\u0431\u043E\u0442\u044B:*
${historyText}

\u{1F517} *\u0412\u0430\u0448\u0430 \u0440\u0435\u0444. \u0441\u0441\u044B\u043B\u043A\u0430:* \`https://lab-gen.pages.dev?ref=${chatId}\``,
      {
        reply_markup: {
          inline_keyboard: [[{ text: "\u{1F48E} \u041F\u041E\u041F\u041E\u041B\u041D\u0418\u0422\u042C \u0411\u0410\u041B\u0410\u041D\u0421", url: "https://t.me/vwzz1" }]]
        }
      }
    );
  } else if (cmd === "\u{1F48E} \u041A\u0423\u041F\u0418\u0422\u042C" || cmd === "/BUY") {
    await sendMessage(
      chatId,
      "\u{1F48E} *\u041F\u041E\u041F\u041E\u041B\u041D\u0415\u041D\u0418\u0415 \u0411\u0410\u041B\u0410\u041D\u0421\u0410*\n\n\u041E\u0434\u043D\u0430 \u0433\u0435\u043D\u0435\u0440\u0430\u0446\u0438\u044F \u2014 \u044D\u0442\u043E \u043F\u043E\u043B\u043D\u044B\u0439 \u043E\u0442\u0447\u0435\u0442: \u043A\u043E\u0434, \u0441\u043A\u0440\u0438\u043D\u0448\u043E\u0442\u044B, \u0434\u0438\u0430\u0433\u0440\u0430\u043C\u043C\u044B \u0438 \u0444\u0430\u0439\u043B DOCX.\n\n\u{1F4B3} *\u0422\u0430\u0440\u0438\u0444\u044B:*\n\u2022 1 \u043B\u0430\u0431\u0430 \u2014 99\u20BD\n\u2022 5 \u043B\u0430\u0431 \u2014 399\u20BD (\u0412\u044B\u0433\u043E\u0434\u0430 20%)\n\u2022 10 \u043B\u0430\u0431 \u2014 699\u20BD (\u0412\u044B\u0433\u043E\u0434\u0430 30%)\n\n\u0414\u043B\u044F \u043E\u043F\u043B\u0430\u0442\u044B \u043D\u0430\u043F\u0438\u0448\u0438\u0442\u0435 \u0430\u0434\u043C\u0438\u043D\u0438\u0441\u0442\u0440\u0430\u0442\u043E\u0440\u0443 \u{1F447}",
      {
        reply_markup: {
          inline_keyboard: [[{ text: "\u{1F468}\u200D\u{1F4BB} \u041D\u0410\u041F\u0418\u0421\u0410\u0422\u042C \u0410\u0414\u041C\u0418\u041D\u0423", url: "https://t.me/vwzz1" }]]
        }
      }
    );
  } else if (cmd === "\u{1F198} \u041F\u041E\u0414\u0414\u0415\u0420\u0416\u041A\u0410" || cmd === "/SUPPORT") {
    const supportText = rawText.split(/\s+/).slice(1).join(" ");
    if (!supportText) {
      await sendMessage(
        chatId,
        "\u{1F4DD} *\u0421\u041B\u0423\u0416\u0411\u0410 \u041F\u041E\u0414\u0414\u0415\u0420\u0416\u041A\u0418*\n\n\u041D\u0430\u043F\u0438\u0448\u0438\u0442\u0435 \u0432\u0430\u0448 \u0432\u043E\u043F\u0440\u043E\u0441 \u043F\u0440\u044F\u043C\u043E \u043F\u043E\u0441\u043B\u0435 \u043A\u043E\u043C\u0430\u043D\u0434\u044B \u0438\u043B\u0438 \u043F\u0440\u043E\u0441\u0442\u043E \u043E\u0442\u043F\u0440\u0430\u0432\u044C\u0442\u0435 \u0441\u043E\u043E\u0431\u0449\u0435\u043D\u0438\u0435.\n\n*\u041F\u0440\u0438\u043C\u0435\u0440:* `/support \u041D\u0435 \u0441\u043A\u0430\u0447\u0438\u0432\u0430\u0435\u0442\u0441\u044F \u0444\u0430\u0439\u043B`"
      );
      return c.json({ ok: true });
    }
    const PROMO_BOT_TOKEN2 = "8726395442:AAFOblOcACvMyFUWn29K73tONsHSrak10S0";
    const adminId = "7403998038";
    await fetch(`https://api.telegram.org/bot${PROMO_BOT_TOKEN2}/sendMessage`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        chat_id: adminId,
        text: `\u{1F198} *\u041E\u0411\u0420\u0410\u0429\u0415\u041D\u0418\u0415*

\u041E\u0442: \`${chatId}\`
\u0422\u0435\u043A\u0441\u0442: ${supportText}`,
        parse_mode: "Markdown",
        reply_markup: {
          inline_keyboard: [[{ text: "\u{1F4AC} \u041E\u0422\u0412\u0415\u0422\u0418\u0422\u042C", callback_data: `reply:${chatId}` }]]
        }
      })
    });
    ...
    if (!rawText.startsWith("/")) {
      await sendMessage(chatId, "\u{1F504} _\u041F\u0435\u0440\u0435\u0441\u044B\u043B\u0430\u044E \u0432\u0430\u0448\u0435 \u0441\u043E\u043E\u0431\u0449\u0435\u043D\u0438\u0435 \u0432 \u043F\u043E\u0434\u0434\u0435\u0440\u0436\u043A\u0443..._");
      const PROMO_BOT_TOKEN2 = "8726395442:AAFOblOcACvMyFUWn29K73tONsHSrak10S0";
      const adminId = "7403998038";
      await fetch(`https://api.telegram.org/bot${PROMO_BOT_TOKEN2}/sendMessage`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ chat_id: adminId, text: `\u{1F4E9} *\u0421\u041E\u041E\u0411\u0429\u0415\u041D\u0418\u0415*

\u041E\u0442: \`${chatId}\`
\u0422\u0435\u043A\u0441\u0442: ${rawText}`, parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "\u{1F4AC} \u041E\u0422\u0412\u0415\u0422\u0418\u0422\u042C", callback_data: `reply:${chatId}` }]] } })
      });
...
// src/routes/promo-bot.ts
var PROMO_BOT_TOKEN = "8726395442:AAFOblOcACvMyFUWn29K73tONsHSrak10S0";
var bot2 = new Hono2();
  ...
  try {
    ...
    if (update.callback_query) {
      ...
      if (data.startsWith("reply:")) {
        ...
        await sendMessage2(adminId, `\u0427\u0442\u043E\u0431\u044B \u043E\u0442\u0432\u0435\u0442\u0438\u0442\u044C \u043F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u044E, \u0432\u0432\u0435\u0434\u0438\u0442\u0435:
...
\`/ans ${targetId} \``);
    ...
    if (cmd === "/id") {
      await sendMessage2(chatId, `Your ID: \`${chatId}\``);
      return c.json({ ok: true });
    }
    if (cmd === "/start") {
      await sendMessage2(chatId, "\u{1F6E0} *\u041F\u0430\u043D\u0435\u043B\u044C \u043F\u0440\u043E\u043C\u043E\u043A\u043E\u0434\u043E\u0432.*\n\n\u0418\u0441\u043F\u043E\u043B\u044C\u0437\u0443\u0439\u0442\u0435 `/promocode <\u0447\u0438\u0441\u043B\u043E>`");
      return c.json({ ok: true });
    }
    if (cmd === "/promocode") {
      const adminId = "7403998038";
      if (String(chatId) !== adminId) {
        await sendMessage2(chatId, `\u26D4 *\u041E\u0442\u043A\u0430\u0437.* \u0412\u0430\u0448 ID (\`${chatId}\`) \u043D\u0435 \u0430\u0434\u043C\u0438\u043D\u0441\u043A\u0438\u0439.`);
        return c.json({ ok: true });
      }
      const amount = parseInt(parts[1]);
      if (isNaN(amount) || amount <= 0) {
        await sendMessage2(chatId, "\u274C *\u041E\u0448\u0438\u0431\u043A\u0430.* \u041D\u0443\u0436\u043D\u043E \u0447\u0438\u0441\u043B\u043E. \u041F\u0440\u0438\u043C\u0435\u0440: `/promocode 10`.");
        return c.json({ ok: true });
      }
      ...
      try {
        ...
        await sendMessage2(chatId, `\u2705 *\u0413\u043E\u0442\u043E\u0432\u043E!*

\u041A\u043E\u0434: \`${promo}\`
\u0417\u0430\u043F\u0440\u043E\u0441\u043E\u0432: *${amount}*`);
      } catch (dbErr) {
        await sendMessage2(chatId, `\u274C *\u041E\u0448\u0438\u0431\u043A\u0430 \u0431\u0430\u0437\u044B:* ${dbErr.message}`);
      }
      return c.json({ ok: true });
    }
    if (cmd === "/stats") {
      ...
      await sendMessage2(
        chatId,
        `\u{1F4CA} *\u0421\u0422\u0410\u0422\u0418\u0421\u0422\u0418\u041A\u0410 LABFORGE*

\u{1F465} \u0412\u0441\u0435\u0433\u043E \u043F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u0435\u0439: *${userCount}*
\u{1F4DD} \u0421\u0433\u0435\u043D\u0435\u0440\u0438\u0440\u043E\u0432\u0430\u043D\u043E \u043B\u0430\u0431: *${labCount}*
\u{1F3AB} \u0418\u0441\u043F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u043D\u043E \u043F\u0440\u043E\u043C\u043E: *${promoCount}*`
      );
      return c.json({ ok: true });
    }
    if (cmd === "/addcredits") {
      ...
      if (!targetId || isNaN(amount)) {
        await sendMessage2(chatId, "\u274C \u041E\u0448\u0438\u0431\u043A\u0430. \u0424\u043E\u0440\u043C\u0430\u0442: `/addcredits <telegram_id> <\u043A\u043E\u043B\u0438\u0447\u0435\u0441\u0442\u0432\u043E>`");
        return c.json({ ok: true });
      }
      const res = await c.env.DB.prepare("UPDATE users SET generations_left = generations_left + ? WHERE telegram_id = ?").bind(amount, targetId).run();
      if (res.meta.changes > 0) {
        await sendMessage2(chatId, `\u2705 \u041D\u0430\u0447\u0438\u0441\u043B\u0435\u043D\u043E *${amount}* \u0437\u0430\u043F\u0440\u043E\u0441\u043E\u0432 \u043F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u044E \`${targetId}\``);
      } else {
        await sendMessage2(chatId, "\u274C \u041F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u044C \u043D\u0435 \u043D\u0430\u0439\u0434\u0435\u043D \u0432 \u0431\u0430\u0437\u0435.");
      }
      return c.json({ ok: true });
    }
    if (cmd === "/broadcast") {
      ...
      if (!text || text === "/broadcast") {
        await sendMessage2(chatId, "\u274C \u0412\u0432\u0435\u0434\u0438\u0442\u0435 \u0442\u0435\u043A\u0441\u0442 \u0440\u0430\u0441\u0441\u044B\u043B\u043A\u0438.");
        return c.json({ ok: true });
      }
      ...
      await sendMessage2(chatId, `\u{1F4E2} \u0420\u0430\u0441\u0441\u044B\u043B\u043A\u0430 \u0437\u0430\u0432\u0435\u0440\u0448\u0435\u043D\u0430. \u041F\u043E\u043B\u0443\u0447\u0438\u043B\u0438: *${count3}* \u0447\u0435\u043B.`);
      return c.json({ ok: true });
    }
    if (cmd === "/users") {
      const adminId = "7403998038";
      ...
      const userList = users.results.map((u) => `\u2022 \`${u.telegram_id}\` (${u.created_at})`).join("\n");
      await sendMessage2(chatId, `\u{1F465} *\u041F\u041E\u0421\u041B\u0415\u0414\u041D\u0418\u0415 \u041F\u041E\u041B\u042C\u0417\u041E\u0412\u0410\u0422\u0415\u041B\u0418:*
...
${userList}`);
    ...
    if (cmd === "/ans") {
      ...
      if (!targetId || !text) {
        await sendMessage2(chatId, "\u274C \u041E\u0448\u0438\u0431\u043A\u0430. \u0424\u043E\u0440\u043C\u0430\u0442: `/ans <ID> <\u0442\u0435\u043A\u0441\u0442>`");
        return c.json({ ok: true });
      }
      const MAIN_BOT_TOKEN = "8608070484:AAFzqPh-XYQnkL6dnDKgpFlA9JxLJrHnkFc";
      await fetch(`https://api.telegram.org/bot${MAIN_BOT_TOKEN}/sendMessage`, {
        ...
        body: JSON.stringify({
          chat_id: targetId,
          text: `\u{1F514} *\u041E\u0422\u0412\u0415\u0422 \u041F\u041E\u0414\u0414\u0415\u0420\u0416\u041A\u0418:*

${text}`,
          parse_mode: "Markdown"
        })
      });
      await sendMessage2(chatId, `\u2705 \u041E\u0442\u0432\u0435\u0442 \u043E\u0442\u043F\u0440\u0430\u0432\u043B\u0435\u043D \u043F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u044E \`${targetId}\``);
      return c.json({ ok: true });
    }
    return c.json({ ok: true });
  } catch (globalErr) {
...
app.route("/api/lab", lab_default);
app.route("/api/bot", bot_default);
app.route("/api/promo-bot", promo_bot_default);
var src_default = app;
...
// .wrangler/tmp/bundle-27uxzS/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-27uxzS/middleware-loader.entry.ts
